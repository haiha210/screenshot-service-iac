...
resource "aws_security_group" "ecs_tasks" {
  ...
  # Allow outbound HTTPS traffic to VPC endpoints (SQS, DynamoDB, S3, ECR, CloudWatch Logs)
  # All AWS services are accessed via VPC endpoints within the VPC CIDR
  egress {
    from_port   = 443
    to_port     = 443
    protocol    = "tcp"
    cidr_blocks = [data.terraform_remote_state.general.outputs.vpc_cidr]
