...
locals {
  ...
  lambda_config_raw = yamldecode(file("${path.module}/../../../../templates/lambda-config.yaml"))

  # Transform to Terraform map structure
  lambda_functions = {
    for fn in local.lambda_config_raw.functions :
    fn.name => {
      handler            = fn.handler
      runtime            = fn.runtime
      timeout            = try(fn.timeout, 30)
      memory_size        = try(fn.memory_size, 128)
      environment        = try(fn.environment, {})
      description        = try(fn.description, "")
      api_path           = try(fn.api_path, null)
      http_method        = try(fn.http_method, "GET")
      enable_api_gateway = try(fn.enable_api_gateway, true)
    }
  }
